...
5. `docs/V3_EXECUTION_MANDATE.md` — single-source governance for V3 work. Supersedes everything else where it conflicts.
...
- **V3 Phase C — vision-native extraction (2026-05-29):** new namespace at `src/mmrag_v3/`. Default entry `src/mmrag_v3/processor.py` → `HybridEngine` cost-optimizer router. Engines: `engines/vlm_native.py`, `engines/vlm_provider.py`, `engines/docling_fast.py` (sole `docling` import boundary in V3), `engines/router.py`. AST firewall at `tests/test_v3_security.py` (13 tests). Default VLM provider is OpenRouter `qwen/qwen3-vl-8b-instruct`; override via `VLM_NATIVE_ENDPOINT` / `VLM_NATIVE_MODEL` / `VLM_NATIVE_API_KEY`. Engine route forced via `USE_VLM_ENGINE=1` or `USE_DOCLING_FAST=1`. Rebaseline utility at `scripts/rebaseline_v3.py`. The V3 Phase A sandbox (chunker / schema / sanitization / Identity Gate) still lives at `v3_execution_root/src/mmrag_v3/`; the gate's subprocess loads the Phase C engine by absolute file path. See `docs/DECISIONS.md` "v3.0 Phase C — Vision-Native Extraction" and `docs/PROJECT_STATUS.md` "Phase C" section.
