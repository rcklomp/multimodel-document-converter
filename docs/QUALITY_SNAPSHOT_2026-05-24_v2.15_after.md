...
| 6 | 3c (UIR refactor) | **PARKED WITH TRIGGERS** (user-explicit disposition 2026-05-24 PM, supersedes the v0.9 plan's force-close recommendation). UIR remains a potentially-useful future refactor for cross-engine adapter standardization; it stays alive as a carry-forward with **four concrete reopen conditions** rather than indefinite-PAUSE or permanent close: (1) 3rd document engine added beyond PDF (Docling) + EPUB (ebooklib); (2) cross-engine chunking defect requiring duplicate fixes in multiple engine paths; (3) per-engine test boilerplate exceeds ~500 LOC duplication; (4) external integration request for HTML/RTF/DOCX. Cycle-open checklist gains "check 3c trigger conditions" line item (30-second review per cycle open). Closes Round-2 Finding 4's concern about perpetual-PAUSE accumulating cognitive load by giving 3c a real-state-with-exit-conditions rather than removing it entirely. |
