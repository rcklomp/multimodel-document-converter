...
| Phase 1 (Form/Table extraction recovery) | **WIN — local NuMarkdown-8B VLM produces clean tables 2026-05-23** | Correction to earlier "0 table chunks" claim: CarOK output DOES emit `docling_table_markdown` chunks; TSR detects tables. The judge's "odd whitespace + truncation" maps to cell-content quality (empty cells in cols 1-3 on some rows, per-page column-count inconsistency, long-description wrapping). Two cheap experiments REGRESSED + reverted: `do_cell_matching=True` (column-collapse + lost word spaces) and `force_full_page_ocr=True` (OCR artifacts + cell-prefix loss). **Local NuMarkdown-8B VLM via `force_table_vlm` solved it**: 5/12 pages got clean 5-column VLM output (vs. OLD 3-4-col merged blobs with empty cells), 6/12 pages kept docling, 1/12 (page 5) VLM returned empty. Page 1 example: OLD merged "480 BMW \| 7,55 TwinPower..." → NEW separates aant/merk/art_nr_merk/ink.ex.BTW/Titel cleanly. **Discovered semantic bug**: `--force-table-vlm` was silently overridden by `technical_manual` profile's `vlm_table_enabled=False` (the profile disables VLM-table to prevent schematic hallucinations). First run with `--force-table-vlm` did nothing; second run with `--profile-override digital_magazine` activated VLM. **Fixed in processor.py 2026-05-23**: `force_table_vlm=True` now truly overrides profile's `vlm_table_enabled=False` (an explicit user flag should force; profile gating remains for the auto-path). Cost: $0 (local NuMarkdown on omlx-server :8000), ~13 min for 12 pages. Cloud `qwen-vl-plus` escalation not needed. |
