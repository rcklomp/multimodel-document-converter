...
|---|---|---|
| Phase 0 (judge calibration, 14B) | **SUPERSEDED 2026-05-23 morning** | First run against retired 14B. Verdict: relevance 81.7% (RESTRICTED), format 90.2% (TRUSTWORTHY), faithfulness 76.1% (RESTRICTED). Report retained as `docs/CALIBRATION_2026-05-22_v2.14_p0_local_judge.md` for historical comparison. |
| Phase 0 (judge calibration, 27B-MTP) | **SUPERSEDED 2026-05-23 afternoon** | Live verdict on the now-retired `Qwen/Qwen3.6-27B-FP8`: relevance 82.0% (RESTRICTED), **format 70.7% (RESTRICTED)**, faithfulness 78.8% (RESTRICTED). All three axes in 70-85% "HyDE-only" band; ±1 agreement 98.6-99.8% (ordinal scale consistent). Bias direction flipped vs the 14B (was lenient on rel/faith, became STRICT on format — 132 downgrades from qwen-max "2" to local "1"). This Format collapse motivated the user's decision to swap to the 30B-A3B-Instruct-2507 endpoint the same afternoon. Verdict body in `docs/CALIBRATION_2026-05-23_v2.14_p0_local_judge_qwen36_27b_mtp.md`. |
| Phase 0 (judge calibration, 30B-A3B-Instruct-2507) | **PENDING — auto-run when endpoint live** | Awaiting user `docker run` of the recipe in [[project-v2-14-gx10-30b-a3b-swap]]. Once health-check passes, `scripts/calibrate_local_judge_vs_qwen_max.py` (defaults now point at the new model) will auto-trigger; ~90 min wall-clock, $0. Verdict will inform whether Phase 4b/4e re-enter scope. |
| Phase 4a (local HyDE) | **SHIPPED 2026-05-22 + re-pointed 2× on 2026-05-23** | `src/mmrag_v2/retrieval/hyde.py` gained `provider="vllm"` knob (5 unit tests). Default model: 14B → 27B-MTP (morning) → 30B-A3B-Instruct-2507 (afternoon). Commit `0c5e818` added `chat_template_kwargs={enable_thinking: False}` to the vLLM payload after the morning Phase 0 debug revealed the 27B was silently dropping content via reasoning-mode routing; 2 bridge tests assert the payload includes the flag for vllm and omits it for dashscope. The 30B-A3B Instruct variant has no `<think>` mode so the kwarg is a defensive no-op there (kept for harness consistency across model swaps). Default provider remains `dashscope` — no behavior change for existing callers. Pending: live HyDE smoke against the 30B-A3B once user runs the swap. **Phase 4 Resilience scope addition (2026-05-23)**: user designated Dashscope `qwen3-max` as the cloud fallback when the GX10 endpoint is unavailable; current `generate_with_fallback` still degrades to literal-query — wiring qwen3-max as a real fallback is a separate change. |
