...
| **REQ-MM-03** | **Contextual Anchoring:** JSONL entry MUST contain `semantic_context` with `prev_text_snippet` (300 chars) and `next_text_snippet` (300 chars). The absence of a digital text layer (scanned documents) is NOT a valid reason for null values. The OCR cascade MUST provide the required anchoring text.|
