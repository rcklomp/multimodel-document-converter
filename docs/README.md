...
2. `docs/QUALITY_SNAPSHOT_2026-05-24_v2.15_after.md`
   - **current canonical baseline** (v2.15.0 SHIPPED 2026-05-24
     under Option F — Phase 3 [F] document-class telemetry suite +
     Phase 1 narrow-fixture sampler on top of unchanged v2.14.x
     state. NO retrieval-stack changes vs v2.14.0 / v2.13.0.
     Phase 1 SOAK EXECUTION deferred-with-evidence per DoD silent-
     default — MLX_API_KEY unavailable in autonomous run.)

2b. `docs/QUALITY_SNAPSHOT_2026-05-23_v2.14_after.md`
   - v2.14.0 predecessor baseline (local-LLM accelerator stack;
     operative Phase 0 FP8-14B per §8 post-ship addendum:
     rel 82.2% / format 90.7% TRUSTWORTHY / faith 76.6%)

2c. `docs/QUALITY_SNAPSHOT_2026-05-22_v2.13_after.md`
   - v2.13.0 predecessor baseline (local omlx embedder swap + OCR
     auto-routing; apples-to-apples 6/6-axis omlx win vs dashscope
     on same fixture)

2d. `docs/QUALITY_SNAPSHOT_2026-05-22_v2.13_p1_omlx_vs_dashscope.md`
   - v2.13 Phase 1 SWAP evidence (canonical comparison report)

2e. `docs/QUALITY_SNAPSHOT_2026-05-21_v2.12_after.md`
...
2f. `docs/QUALITY_SNAPSHOT_2026-05-20_v2.11_soak_qwen3.md`
   - v2.11.0 baseline soak; the 518-query × 259-chunk fixture every
     v2.12 phase soak ran against

2g. `docs/QUALITY_SNAPSHOT_2026-05-16_v2.10_after.md`
   - v2.10 strict-gate baseline (corpus 34/34 PASS; unchanged in
     v2.11 + v2.12 + v2.13 + v2.14 + v2.15 — all five cycles changed
     only the retrieval side / OCR routing / local-LLM accelerators /
     telemetry observability)

3. `docs/PLAN_V2.15.md`
   - v2.15 execution history — CLOSED 2026-05-24 under Option F;
     tag staged for push. Draft v0.9 + 8-round audit archaeology
     in Appendix A.

3b. `docs/CYCLE_OPEN_CHECKLIST.md`
   - NEW in v2.15 — cycle-open process: analyzer run + USER_ISSUES
     review + Docling watcher + calibration freshness +
     cycle_slip.log spec. Read FIRST at v2.16 open.

3c. `docs/PLAN_V2.14.md`
...
Read `docs/ARCHITECTURE.md` when changing core pipeline behavior. **Current canonical baseline is `docs/QUALITY_SNAPSHOT_2026-05-24_v2.15_after.md`** — v2.15.0 ship state (Option F: Phase 3 document-class telemetry suite + Phase 1 narrow-fixture sampler on top of unchanged v2.14.x state; NO retrieval-stack changes vs v2.13.0). Production embedder is local `Qwen3-Embedding-8B-mxfp8` via omlx-server (`10.0.10.246:8000`) against `mmrag_v2_8__qwen3_local` (4096-dim, 31,371 pts); the dashscope rollback baseline `mmrag_v2_8__qwen3_dashscope` was **dropped 2026-05-23 PM** (v2.14 Phase 3 under user "full send" override), with 90-day cold-storage snapshot retained on `multimodal-doc-converter_qdrant_snapshots` through 2026-08-21. Production local LLM is `RedHatAI/Qwen2.5-14B-Instruct-FP8-dynamic` at `http://10.0.10.239:8000/v1/chat/completions` (Asus Ascent GX10; Blackwell-native FP8; bare config — n-gram spec REJECTED). v2.15 added Option F telemetry: `scripts/analyze_doc_class_telemetry.py` reads `output/telemetry/document_class_hits.jsonl` at cycle-open per `docs/CYCLE_OPEN_CHECKLIST.md`. Strict-gate state from v2.10 Phase 8 (`docs/QUALITY_SNAPSHOT_2026-05-16_v2.10_after.md`) is unchanged because v2.11 / v2.12 / v2.13 / v2.14 / v2.15 all touched retrieval-side / OCR-routing / observability only, not extraction / chunking / validation. The v2.11.0 Format-gate downgrade to ≥85% (see `docs/DECISIONS.md` "v2.11.0 Embedder Swap Executed — Format Gate Downgrade") was structurally addressed in v2.13 via OCR auto-routing (Earthship +6.2pp) + the omlx embedder Format lift (+3.7pp on same-fixture comparison); the remaining CarOK + Fluent_Python Format/code-extraction defects are now tracked as Option F documented-limitation classes with telemetry-driven v2.16+ adjudication per `docs/DECISIONS.md` "v2.15 Documented-Limitation Telemetry Threshold (ACTIVE RULE)". When a metric appears in both a layer-1 status doc and a dated snapshot, the latest snapshot is canonical (per `docs/AGENT_GOVERNANCE.md` Canonicality Rule). The archived task logs are archaeology only — current task state lives in `docs/PROJECT_STATUS.md`.
