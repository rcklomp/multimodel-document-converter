...
**2026-05-10 RC amendment.** Phase 4 closed with two explicitly
signed-off v2.10 deferrals: Firearms `OCR_PATH_HEADING_PROPAGATION`
and KI EPUB `KI_EPUB_EXTRACTION_LANE_REWRITE`. The original
`v2.9.0` production tag bar remains stricter and still requires
Firearms `HEADING >= 0.80` plus KI EPUB `UNIVERSAL_PASS`. Phase 5 may
now run as a `v2.9.0-rc1` evidence pass with those two known
limitations documented; it must not be represented as a final
`v2.9.0` completion claim.
...
1. `scripts/qa_full_conversion.py output/<v29_run>/<doc>/ingestion.jsonl`
   ...
   For `v2.9.0-rc1` only, two non-PASS rows are allowed when and only
   when they match the signed-off Phase 4 deferrals: Firearms HEADING
   below 0.80 from `OCR_PATH_HEADING_PROPAGATION`, and KI EPUB
...
3. `Firearms` v2.9 fresh: `HEADING coverage ≥ 0.80` AND content
   ...
   For `v2.9.0-rc1` only, this is documented as a known limitation:
   observed Phase 4 baseline `HEADING=0.722`, strict target unchanged,
   fix deferred to v2.10.
...
8. `scripts/convert_books_v29.py` runs **without** `--no-refiner` and
   ...
   The legacy shell runner is disabled for Phase 5 because nested
   shell execution makes torch report `mps=False` on Apple Silicon.

### Non-Goals (deferred to v2.10 or later — verbatim from prompt §2)

- **Firearms OCR-path heading propagation.** Deferred to v2.10 as
  `OCR_PATH_HEADING_PROPAGATION` with explicit user sign-off on
  2026-05-10. The strict `HEADING >= 0.80` target is not relaxed for
  the final `v2.9.0` tag; the `v2.9.0-rc1` Phase 5 run records the
  current 0.722 baseline as a known limitation.
- **KI EPUB extraction lane rewrite.** Deferred to v2.10 as
  `KI_EPUB_EXTRACTION_LANE_REWRITE` with explicit user sign-off on
  2026-05-10. The `UNIVERSAL_PASS` requirement is not relaxed for the
  final `v2.9.0` tag; the `v2.9.0-rc1` Phase 5 run records the
  existing structural failure as a known limitation.
...
| Phase 3 | Resolve `IMAGE_DESCRIPTION_UNUSABLE` policy/model behavior — execution plan in `docs/archive/PLAN_V2.9__STEP3.md` | `complete` (2026-05-09, commits `649c952` + `51e897b`) |
| Phase 4 | Resolve localized hard failures (Combat, Adedeji, Devlin, Earthship, Firearms, KI_En_ChatGPT) — Phase 2 added Firearms HEADING coverage and chunk-count drift to scope. Closure evidence in `docs/archive/PLAN_V2.9__PHASE4.md` and `docs/QUALITY_SNAPSHOT_2026-05-09_v2.9_phase4_after.md`. | `closed with signed deferrals` (2026-05-10): Firearms HEADING (`OCR_PATH_HEADING_PROPAGATION`) and KI EPUB (`KI_EPUB_EXTRACTION_LANE_REWRITE`) both deferred to v2.10 |
| Phase 5 | Broad reconversion, enrichment, Qdrant drop/recreate, RC AFTER snapshot | `blocked in 5c` (2026-05-11): 34/34 fresh outputs and Phase 5b VLM acceptance pass; Qdrant service/Docker access unavailable |
...
> **Execution plan: `docs/archive/PLAN_V2.9__STEP3.md`** (split out 2026-05-08
...
### Phase 4 — Localized strict-gate hard failures — `closed with signed deferrals` (2026-05-10)
...
User sign-off recorded 2026-05-10 for `v2.9.0-rc1` execution only.
Detailed closure plan + evidence in `docs/archive/PLAN_V2.9__PHASE4.md` and
...
> **Execution status 2026-05-11:** Phase 5a broad reconversion is
> complete for 34/34 canonical docs, and Phase 5b cloud VLM enrichment
> passes the env-gated acceptance test. Phase 5c is blocked because
> local Qdrant is not reachable on `localhost:6333` and Docker socket
> access could not be approved in this environment. See
> `docs/QUALITY_SNAPSHOT_2026-05-10_v2.9_phase5_attempt.md`.

**What:** With current Phases 0–4 closed and two Phase 4 deferrals
signed off for `v2.9.0-rc1`, run the broad reconversion that
...
clean, and produce the `v2.9.0-rc1` AFTER snapshot.
...
For the RC, Firearms HEADING and KI EPUB universal failures are
accepted only as tracked known limitations. Any additional strict-gate
failure remains a blocker for 5b/5c.
...
- [ ] Phases 0-4 all merged; Phase 4 deferrals recorded; no final
      `v2.9.0` tag or completion claim exists before Phase 5 finishes.
...
- [ ] Alibaba DashScope API key reachable; Source Sanctity
      ...
      Combat-style hallucinations = 0; per archived
      `docs/archive/PROGRESS_CHECKLIST.md` Workstream A) are
      **reference points, not hard caps for v2.9**
...
|---|---|---|---|
| Conversion runner | `scripts/convert_books_v29.py` | 34 entries; v2.9 prep removed the `--no-refiner` workaround; runner refuses to start if its Python process cannot see MPS | Keep `--vision-provider none --no-cache`; Phase 5's targeted enrichment script handles VLM, not conversion-time VLM. |
...
**Execution evidence (2026-05-10/11):**

- Runner: `conda run -n mmrag-v2 python scripts/convert_books_v29.py`.
  Direct conda Python reports MPS available; nested shell runners make
  torch report `mps=False`, so `scripts/convert_books.sh` is disabled
  for Phase 5 unless explicitly overridden.
- Fresh successes: 34/34 canonical docs have new
  `output/<doc>/ingestion.jsonl` from the Phase 5 run.
- Corpus shape: 30,356 chunks / 30,356 unique chunk_ids.
- Long-tail conversions required a 10,800-second per-doc cap:
  `Bourne_RAG_2024`, `Raieli_AI_Agents`, `Hao_ML_Platform`, and
  `Greenhouse_Design` completed under the hardened runner.
- Runner hardening after the Greenhouse hang: each child conversion
  now starts in a separate process group; timeout sends SIGTERM to
  the group, escalates to SIGKILL after 30 seconds, logs `TIMEOUT`,
  and continues only when `--keep-going` is set. Per-target lock files
  now prevent concurrent conversions from writing the same output dir.
- Phase 5b cloud `qwen3-vl-plus` enrichment completed across the
  canonical corpus: 4,382 image chunks processed. Acceptance:
  `RUN_V29_VLM_ACCEPTANCE=1 pytest tests/test_v29_image_enrichment_acceptance.py -q`
  → `1 passed`.
- Phase 5c is blocked: `curl http://localhost:6333/collections` cannot
  connect, and Docker socket access was denied/rejected by the
  platform usage limit. `mmrag_v2_8` has not been dropped/recreated.

**Parallelism note:** `scripts/convert_books_v29.py` runs sequentially
...
1. Run `conda run -n mmrag-v2 python scripts/convert_books_v29.py`
   with the existing v2.9 flag posture. `--no-refiner` was already
   removed in `538cf89`; verify it remains absent rather than
   re-doing that change. Do not use `bash scripts/convert_books.sh`
   for Phase 5; it is intentionally disabled unless
   `MMRAG_ALLOW_LEGACY_SHELL_RUNNER=1` is set because nested shell
   execution loses MPS visibility on Apple Silicon.
...
3. Targeted verification:
   ...
   - **Firearms:** `profile_type=scanned` (or `scanned_degraded`).
     For final `v2.9.0`, HEADING coverage ≥ 0.80 and chunk count
     within ±2% of 1690. For `v2.9.0-rc1`, record the observed
     HEADING baseline and the signed `OCR_PATH_HEADING_PROPAGATION`
     v2.10 deferral.
...
4. If any target fails outside the two signed RC limitations,
   **stop**. Do not proceed to 5b/c. Diagnose and fix in the
...
1. Create `docs/QUALITY_SNAPSHOT_<rc_date>_v2.9_rc1_after.md`
   ...
   - Known Limitations with both signed v2.10 deferrals and their
     measured RC baselines.
2. Update `docs/PROJECT_STATUS.md` "Active Baseline" / "Current
   Objective" pointers to the new RC snapshot without marking
   `v2.9.0` shipped.
3. Do not recreate `docs/PROGRESS_CHECKLIST.md`; it was archived on
   2026-05-07. Treat remaining checklist references as
   `docs/archive/PROGRESS_CHECKLIST.md` historical pointers and use
   `PROJECT_STATUS.md` as the current source of truth.
4. Update `CHANGELOG.md` with an `[2.9.0-rc1] — <rc_date>` entry:
   Added / Changed / Fixed / Known Limitations for each phase.
5. Do not bump `__engine_version__` to `2.9.0` for the RC unless a
   separate release decision is made. Schema version stays `2.7.0`.
6. Tag `v2.9.0-rc1` after the RC evidence is tracked. Tag `v2.9.0`
   only when the 6 binding requirements from
...
- The Firearms verify test from Phase 4
  ...
  remains the final `v2.9.0` contract. Do not run it as an RC gate;
  record the RC baseline instead.
...
**Done when (`v2.9.0-rc1`):**
- All §2 Goals are met empirically except the two signed RC
  limitations, which are measured and documented.
- v2.9 RC AFTER snapshot exists and `docs/PROJECT_STATUS.md` "Current
  Objective" points at it.
- v2.8 AFTER snapshot
...
- `v2.9.0-rc1` annotated tag on the RC AFTER-snapshot commit. Final
  `v2.9.0` remains blocked until Firearms and KI EPUB meet the
...
The final `v2.9.0` plan is "done" when every signal below passes.
The `v2.9.0-rc1` Phase 5 run uses the same commands, but accepts only
the two signed Phase 4 limitations: Firearms HEADING below 0.80 and
KI EPUB `UNIVERSAL_FAIL`. No other non-PASS is allowed.
...
# expected for final v2.9.0: QA_PASS for all 34, except explicit
# documented form/document-class variants allowed by docs/QUALITY_GATES.md.
# expected for v2.9.0-rc1: same, except the two signed limitations
# may remain non-PASS and must be recorded in the RC AFTER snapshot.
...
# expected for final v2.9.0: UNIVERSAL_PASS on all 34.
# expected for v2.9.0-rc1: UNIVERSAL_PASS on all 34 except the
# signed KI EPUB extraction-lane limitation.
...
# 9. RC tag
git tag v2.9.0-rc1
# final v2.9.0 tag only after the 6 Completion Rules below are all
# satisfied without the RC-only limitations.
```
...
- **Ayeva:** `profile_type=technical_manual`,
  `indentation_fidelity ≥ 0.85`, CODE PASS.
- **Firearms:** `profile_type=scanned` (or `scanned_degraded`),
  HEADING coverage ≥ 0.80 for final `v2.9.0`; for `v2.9.0-rc1`,
  record the measured baseline and the signed v2.10 deferral.
...
`v2.9.0-rc1` is an evidence tag, not a completion tag. It may be cut
with the two signed known limitations documented. The final `v2.9.0`
tag must satisfy the completion rules below without those RC-only
exceptions.
...
2. Will satisfy when the v2.9 AFTER snapshot is `tracked` in `docs/`; outputs are
   `local-run` with commands recorded in §3 / `convert_books_v29.py`.
...
5. Will satisfy when `PROJECT_STATUS.md` "Active Baseline" points at the v2.9
   AFTER snapshot; archived `docs/archive/PROGRESS_CHECKLIST.md`
   references are historical only; the v2.8 AFTER snapshot is
   banner-marked superseded per the Canonicality Rule.
6. Will satisfy when a fresh agent reading `PROJECT_STATUS.md` →
   `AGENTS.md` → `docs/QUALITY_SNAPSHOT_<v29>_after.md` reproduces the v2.9
...
| **HybridChunker per-item token guard** | Requires upstream Docling work. | Milestone 1 known limitation in `docs/archive/PROGRESS_CHECKLIST.md` |
...
| **Automated baseline delta reporter** (`scripts/qa_delta_report.py`) | Useful tooling but not a blocker; manual diff against v2.8 AFTER suffices for v2.9. | `docs/archive/PROGRESS_CHECKLIST.md` Baseline And Tracking |
...
- `docs/PROJECT_STATUS.md` — current source of truth for phase status,
  evidence path, and test counts. Do not recreate
  `docs/PROGRESS_CHECKLIST.md`; it was archived on 2026-05-07.
...
- **2026-05-10 v2.1 (decision k)** — Phase 4 Firearms HEADING and KI
  EPUB deferrals signed off for `v2.9.0-rc1` execution. Phase 5 was
  unblocked for RC evidence, then Phase 5a exposed four conversion
  blockers; final `v2.9.0` remains blocked by the unchanged
...
- **2026-05-06 v2.0 (decision j)** — phase order revised around the
  ...
  resolves localized hard failures; current Phase 5 runs only after
  blockers are either fixed or explicitly signed off as known
  limitations for an RC.
