...
**Engine version:** v2.12.0 (schema version 2.7.0 — chunk-shape contract unchanged since v2.7; v2.8 → v2.9 → v2.10 → v2.11 → v2.12 are all behavioral / quality-bar / retrieval-side changes, not schema changes).
**Date:** May 2026
**Status:** PRODUCTION — v2.12.0 SHIPPED 2026-05-21, annotated tag `5a2ce18` public on both `github` (rcklomp/multimodel-document-converter) and `origin` (Gitea at 10.0.10.241). **v2.13 cycle in flight (autonomous run started 2026-05-22):** Phase 2 (Format recovery for Earthship + Firearms scanned-OCR) shipped; Phase 1 (local Qwen3-Embedding-8B-mxfp8 swap candidate) rebuilding parallel collection in background. See `docs/PLAN_V2.13.md`. Predecessors: `v2.12.0` (2026-05-21, `5a2ce18`), `v2.11.0` (2026-05-20, `c2a461c`), `v2.10.0` (2026-05-16, `db6527c`), `v2.9.0-rc1` (2026-05-12, `3e06d1b`), `v2.8.0` (2026-05-04, `645ab2b`).
**Policy Update (v2.12.0):** Production retrieval pipeline now uses hybrid retrieval (BM25 sparse + dense + RRF fusion) plus a local cross-encoder reranker (`gte-reranker-modernbert-base-mlx` via omlx-server). New `src/mmrag_v2/retrieval/` module is the canonical entry point — see `retrieve_hybrid_reranked()` in `pipeline.py`. New sparse-only Qdrant side-collection `mmrag_v2_8__bm25_sparse` (25,623 chunks, status green) paired with the existing dense collection. BM25 index tracked at `tests/fixtures/bm25_index_v2_12.json` (deterministic, reproducible). HyDE ships opt-in (default off); use `use_hyde=True` to enable. Cumulative lift vs v2.11.0: Recall@1 chunk 35.5% → 67.8%; Recall@5 chunk 66.8% → 90.2% STRETCH; Recall@5 doc 91.7% → 98.6% STRETCH; Faithfulness 50.6% → 72.6%. End-to-end p99 latency ~2.05 s. Format remains at 88.4% (chunk-level OCR/form-shape damage; v2.13 carry-forward). See `docs/DECISIONS.md` "v2.12 Phase 1/2/3 Outcome" sections and `docs/QUALITY_SNAPSHOT_2026-05-21_v2.12_after.md`.
...
This document describes the current MM-Converter v2.12.0 architecture for robust multimodal ingestion across supported formats. (Section headers below tagged "(v2.7.0)" reflect when individual subsystems were introduced; they are still accurate descriptions of current behavior unless explicitly noted. v2.11.0 changed the *retrieval-side* embedder; v2.12.0 added the retrieval-side stack on top — cross-encoder reranker, BM25 sparse, RRF fusion. Text/image extraction, chunking, validation, and enrichment lanes are unchanged from v2.10.)
