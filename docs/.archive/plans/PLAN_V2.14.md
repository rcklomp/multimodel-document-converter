...
| Phase 6 (code-block chunking hygiene) | **PARTIAL — observability shipped 2026-05-23; deeper fix deferred to v2.15** | **Committed (d737147):** block-extension policy + `partial_code` schema field + 3 new tests on the `_chunk_text_with_overlap` path. Improves the `scanned_book` lane + adds universal observability. Tests: 9/9 in `tests/test_code_chunking.py`; suite 1036/1036. **Investigated this cycle (not shipped):** HybridChunker post-merge pass for cross-page CODE+CODE severances. Implementation is correct (unit tests pass; offline simulation finds 7-11 candidates in Fluent_Python) but fires 0× in production due to chunks-list-state subtleties I couldn't pin down in finite time. More importantly, the **actual defect shape on Fluent_Python is Docling-extraction-layer prose+code mixing, NOT classical CODE+CODE severance**: page 326 CODE chunk ends with `    return`; the continuation `cls(memv)\n\`\`\`\n- Import itertools...` is the first 11 chars of a PARAGRAPH chunk that then explains the code. A CODE+CODE merge can't catch this; surgical prose-prefix extraction would have unacceptable false-positive risk (any prose with colons or code-words would trigger). **The actual fix is upstream of chunking** — either Docling configuration tuning to preserve cleaner CodeItem boundaries OR a post-Docling text-normalization step that re-inserts whitespace around code fences. Both are v2.15 workstreams. Evidence + offline-simulation harness preserved in this session's experiments. |
| Phase 1 (Form/Table extraction recovery) | **PARTIAL — extraction works, dual-emission blocks retrieval surfacing; v2.15 dedup needed** | **Code/extraction side WORKS** (commit `e60a253` semantic-bug fix kept): `--force-table-vlm` now truly forces (was silently overridden by `technical_manual` profile's `vlm_table_enabled=False`); local NuMarkdown-8B VLM produces clean 5-column markdown tables on 5/12 CarOK pages ($0, ~13 min). **BUT mini-soak validation (2026-05-23) failed the Phase 1 acceptance bar**: 30-query CarOK soak against re-ingested chunks measured Format 45.0% (was 71.9% baseline; **-26.9pp REGRESSION**) while Relevance +24.2pp and Faithfulness +21.7pp improved. Root cause: `force_table_vlm=True` ADDS a VLM-table chunk per page but does NOT remove the parallel flat-text `hybrid_chunker` prose chunk that Docling emits for the same page. Both chunks coexist in Qdrant; hybrid+RRF+rerank picks the prose chunk 29/30 times (0 VLM tables ever won top-1). The flat prose chunks are still ugly → Format judged 0/1; the VLM tables are dead weight. **Production state rolled back to v2.13 baseline**: OLD CarOK chunks re-ingested into dense (81 chunks) + sparse rebuilt with OLD corpus (70 chunks); verified counts match initial. **v2.15 follow-up needed**: when VLM produces a table chunk for a page, suppress the parallel prose-chunk emission for that page so the VLM is the only candidate. Two cheap pre-experiments also REGRESSED + reverted: `do_cell_matching=True` (column-collapse + lost word spaces) and `force_full_page_ocr=True` (OCR artifacts + cell-prefix loss). |
...
| Phase 4c (local query gen) | **SHIPPED 2026-05-23** (commit `1c201dd`) | `synthetic_soak.py --gen-provider vllm` + payload-side `chat_template_kwargs.enable_thinking=False`. Live smoke: 2.0s/query against the 27B, $0. Default stays `dashscope`. |
| Phase 4d (tie-breaker harness) | **SHIPPED 2026-05-23** | `scripts/local_then_cloud_soak.py` + 14 unit tests in `tests/test_local_then_cloud_soak.py`. Two-tier judging: local-vLLM judges every in-scope query (free); cloud `qwen-max` re-judges only "contested" queries (any axis < `--contested-axis-floor`, default 2). Final scoring uses cloud judgment when present, local otherwise; provenance tagged via `judgment.judge_source` ∈ {`local`, `cloud`, `local_fallback`}. Live smoke: 4 local judgments in 13s, work file unchanged byte-for-byte. Report at `docs/SOAK_<date>_v2.14_p4d_tiebreaker.md` shows agreement on the re-judged set + cost savings vs full-cloud. |
