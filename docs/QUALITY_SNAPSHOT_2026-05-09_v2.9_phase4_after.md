...
> **Status:** Phase 4 closed pending TWO sign-offs (Step 4 + Step 6).
> Steps 1, 2, 3, 5 shipped; Steps 4 + 6 deferred to v2.10 with
> sign-off pending. Phase 4 Step 4 Path A was briefly shipped
...
| 4 (decision) | Firearms HEADING → DEFER to v2.10 (sign-off pending) | (this snapshot) | — |
...
| Firearms (no VLM) | HEADING FAIL (72 % < 80 %) | HEADING FAIL unchanged — DEFERRED | Step 4 deferral pending sign-off; carry-forward fix `b429cb5` shipped but doesn't apply (Firearms is OCR-routed, not HybridChunker-routed) |
...
### Firearms HEADING — DEFERRED (sign-off pending)
...
- [ ] **Pending:** Firearms HEADING ≥ 0.80. **Not met** — Firearms still FAILs the gate. Deferral to v2.10 (`OCR_PATH_HEADING_PROPAGATION`) requested with explicit user sign-off.
...
The correct action — taken in `cbd7fb4` — was to revert the gate change and defer Firearms HEADING to v2.10 as a sign-off-pending item, parallel to the existing KI EPUB deferral.
