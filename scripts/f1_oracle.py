...
def _write_artifacts(post: list[dict[str, Any]], book: str, out_dir: Path, n_pages: int) -> int:
    ...
    with art.open("w", encoding="utf-8") as fh:
        ...
        for pno in pages:
            for ch in by_page[pno]:
                ...
                fh.write(f"===== page {pno}  chunk {ch.get('chunk_id')}  [{parses}] =====\n")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    pages = _write_artifacts(post, args.book, Path(args.artifacts_dir), args.artifact_pages)
