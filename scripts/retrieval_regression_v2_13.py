#!/usr/bin/env python3
"""v2.13 retrieval-regression harness — hybrid + rerank (omlx embedder).

Pins the v2.13.0 production retrieval shape against the live stack
...
fingerprints coexist as cycle-close archaeology:
  ...
  retrieval_regression_v2_13_hybrid.json  v2.13.0 (hybrid + rerank, omlx)

The v2.12 fingerprint stays pinned to dashscope (contract for that
release). The v2.13 fingerprint pins the new omlx-based production
shape. Both regression tests run on the live stack pre-tag; v2.13 is
...
FIXTURE_PATH_DEFAULT = REPO_ROOT / "tests" / "fixtures" / "retrieval_regression_v2_13_hybrid.json"
...
def capture(fixture_path: Path) -> None:
    ...
    print(f"\nWrote v2.13 fingerprint: {fixture_path}")
...
def verify(fixture_path: Path) -> int:
    ...
    print(f"\nv2.13 hybrid regression: {pass_count}/{total} PASS")
