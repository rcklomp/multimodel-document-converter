...
def stage_retrieve(work_path: Path, qdrant_url: str, ollama_url: str) -> None:
    rows = _read_work(work_path)
    if not rows:
    ...
    for r in rows:
        for q in (r.get("queries") or []):
            if q.get("retrieval"):
                continue
            try:
                vec = embed(q["query_text"], model=EMBED_MODEL, ollama_url=ollama_url)
...
def stage_report(work_path: Path, report_path: Path) -> None:
    rows = _read_work(work_path)
    if not rows:
    ...
    lines: list[str] = []
    lines.append(f"# Quality Snapshot {now.strftime('%Y-%m-%d')} — v2.10.0-rc1 SOAK")
    lines.append("")
    lines.append("> **Status:** synthetic-soak report (Phase 9 candidate).")
    lines.append(f"> Source: `{work_path.relative_to(REPO_ROOT)}`.")
    lines.append(f"> Judge: Dashscope `{JUDGE_MODEL}`. Generator: `{GENERATOR_MODEL}`. Embedder: `{EMBED_MODEL}` (Ollama).")
    lines.append("> No QA threshold; this snapshot is informational, intended to seed v2.10.x defect candidates.")
    lines.append("")
    ...
    lines.append(f"- Each query → top-{TOP_K} retrieved from `{COLLECTION}` via Ollama `{EMBED_MODEL}` 4096-dim embeddings.")
...
def main() -> int:
    ...
    needs_key = args.stage in ("generate", "judge", "all")
    if needs_key and not api_key:
        print("ERROR: DASHSCOPE_API_KEY env var is not set; required for generate/judge stages.", file=sys.stderr)
        return 2
